        ...
        <div class="row">
            <div class="col">
                <h4 class="h5 align-middle m-0 font-weight-bold text-danger">
                    Rekap Pick-Up Luar Negeri (Berdasarkan Mitra)
                </h4>    
    ...
    <div class="table-responsive">
        <table class="table table-striped w-100 nowrap" id="dataTable">
            ...
            <tbody>
                ...
                from pu_ln INNER JOIN mitra ON pu_ln.mitra_id = mitra.id_mitra WHERE tgl_masuk BETWEEN '$from' AND '$end' group by mitra_id");
                $sum = mysqli_query($con,"select sum(ems_jml) as jml_ems, sum(pos_expt_jml) as jml_pos_expt, sum(pp_cpt_jml) as jml_pp_cpt, sum(pp_laut_jml) as jml_pp_laut, sum(r_ln_jml) as jml_r_ln,  sum(e_pack_jml) as jml_e_pack, sum(lain_jml) as jml_lain, sum(total) as jml_total from pu_ln WHERE tgl_masuk BETWEEN '$from' AND '$end'");
                $sum = $sum->fetch_assoc();
                $no = 1;
                if ($query) :
                    foreach ($query as $km) :
