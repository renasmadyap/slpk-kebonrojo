    ...
    <div class="table-responsive">
        <table class="table table-striped w-100 nowrap" id="dataTable1">
            ...
            <tbody>
                ...
                $nama = mysqli_query($con, "select nama_petugas from pu_ln where nippos = '$nippos' group by nippos");
                $row = mysqli_fetch_array($nama);
                $_SESSION['nama_pet'] = $row['nama_petugas'];
                $query=mysqli_query($con,"select *, sum(ems_jml) as jml_ems, sum(pos_expt_jml) as jml_pos_expt, sum(pp_cpt_jml) as jml_pp_cpt, sum(pp_laut_jml) as jml_pp_laut, sum(r_ln_jml) as jml_r_ln, sum(e_pack_jml) as jml_e_pack, sum(lain_jml) as jml_lain, sum(total) as jml_total from pu_ln INNER JOIN mitra ON pu_ln.mitra_id = mitra.id_mitra WHERE nippos = '$nippos' AND tgl_masuk BETWEEN '$from' AND '$end' group by mitra_id");
                $sum = mysqli_query($con,"select sum(ems_jml) as jml_ems, sum(pos_expt_jml) as jml_pos_expt, sum(pp_cpt_jml) as jml_pp_cpt, sum(pp_laut_jml) as jml_pp_laut, sum(r_ln_jml) as jml_r_ln,  sum(e_pack_jml) as jml_e_pack, sum(lain_jml) as jml_lain, sum(total) as jml_total from pu_ln WHERE nippos = '$nippos' AND tgl_masuk BETWEEN '$from' AND '$end'");
                $sum = $sum->fetch_assoc();
                $_SESSION['nama'] = $_SESSION['nama'] = mysqli_query($con,"select * from pu_ln WHERE nippos = '$nippos' group by nippos");
                $no = 1;
                if ($query) :
                    foreach ($query as $km) :
                        ?>
