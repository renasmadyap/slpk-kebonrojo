        ...
        <div class="row">
            <div class="col">
                <h4 class="h5 align-middle m-0 font-weight-bold text-danger">
                    Rekap Data Koreksi Dalam Negeri
                </h4> 
    ...
    <div class="table-responsive">
        <table class="table table-striped w-100 nowrap" id="dataTable1">
            ...
            <tbody>
                ...
                $nama = mysqli_query($con, "select nama_petugas from mandor_koreksi where nippos = '$nippos' group by nippos");
                $row = mysqli_fetch_array($nama);
                $_SESSION['nama_pet'] = $row['nama_petugas'];
                $query=mysqli_query($con,"select 
                *, sum(instan_jml) as jml_instan, sum(pos_exp_jml) as jml_posex, sum(pkh_jml) as jml_pkh, sum(perlaksus_jml) as jml_perlaksus, sum(log_jml) as jml_log, sum(lain_jml) as jml_lain, sum(total) as jml_total from mandor_koreksi INNER JOIN mitra ON mandor_koreksi.mitra_id = mitra.id_mitra WHERE nippos = '$nippos' AND tgl_masuk BETWEEN '$from' AND '$end' group by mitra_id");
                $sum = mysqli_query($con,"select 
                *, sum(instan_jml) as jml_instan, sum(pos_exp_jml) as jml_posex, sum(pkh_jml) as jml_pkh, sum(perlaksus_jml) as jml_perlaksus, sum(log_jml) as jml_log, sum(lain_jml) as jml_lain, sum(total) as jml_total from mandor_koreksi WHERE nippos = '$nippos' AND tgl_masuk BETWEEN '$from' AND '$end'");
                $sum = $sum->fetch_assoc();
                $no = 1;
                if ($query) :
                    foreach ($query as $km) :
